...
trait QAcademicYear extends QId[Long]  {
...
object QAcademicYear {
  def apply(parent: PersistableExpression[_], name: String, depth: Int): QAcademicYear = {
    new PersistableExpressionImpl[AcademicYear](parent, name) with QId[Long] with QAcademicYear
  }
  
  def apply(cls: Class[AcademicYear], name: String, exprType: ExpressionType): QAcademicYear = {
    new PersistableExpressionImpl[AcademicYear](cls, name, exprType) with QId[Long] with QAcademicYear
  }
