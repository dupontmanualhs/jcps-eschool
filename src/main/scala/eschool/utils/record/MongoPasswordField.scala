 ...
 *
 * Modified by Tyler Darnell for jcps-eschool
 */
...
object MongoPasswordField {
  ...
  def encrypt(s: String, salt: String, alg: String) = {
    val md = MessageDigest.getInstance(alg)
    md.update(salt.getBytes)
    val hashed = md.digest(s.getBytes)
    alg + "$" + salt + "$" + new String(hashed)
  }
}

class MongoPasswordField[OwnerType <: BsonRecord[OwnerType]](rec: OwnerType, minLen: Int, alg: String) extends JsonObjectField[OwnerType, Password](rec, Password) {
  ...
  override def set_!(in: Box[Password]): Box[Password] = {
    ...
    in.map(p =>
      if (p.salt.length == 0) // only encrypt the password if it hasn't already been encrypted
        Password(MongoPasswordField.encrypt(p.pwd, salt_i.get, alg), salt_i.get)
      else
        p
    )
  }
  ...
  def isMatch(toMatch: String): Boolean = {
    val parts = value.pwd.split('$')
    MongoPasswordField.encrypt(toMatch, parts(1), parts(0)).split('$')(2) == parts(2)
  }
}